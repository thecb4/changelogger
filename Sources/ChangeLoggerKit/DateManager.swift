...
extension Date {
  public func string(
    using formatter: DateFormatter = DateManager.formatter,
    with format: String = DateManager.defaultFormat
  ) -> String {
  ...
  public var iso8601StringWithFullNanosecond: String {
    ...
    if nanosecond != 0 {
      return DateManager.iso8601WithoutZFormatter.string(from: self) +
        String(format: ".%09d", nanosecond).trimmingCharacters(in: characterSetZero) + "Z"
    } else {
      return DateManager.iso8601Formatter.string(from: self)
    }
  }
}
